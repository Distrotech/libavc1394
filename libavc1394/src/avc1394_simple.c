...
int avc1394_is_subunit_recorder(raw1394handle_t handle, nodeid_t node)
{
    quadlet_t table[8];
    int i, j;
    int entry;
    ...
    for (i=0; i<8; i++) {
        for (j=3; j>=0; j--) {
            ...
            id = entry
                 >> 3;
            if (id == (AVC1394_SUBUNIT_TYPE_TAPE_RECORDER >> 19) ||
                    id == (AVC1394_SUBUNIT_TYPE_DISC_RECORDER >> 19))
                return 1;
        }
    }
    return 0;
